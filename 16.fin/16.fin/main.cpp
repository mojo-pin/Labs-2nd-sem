...
{
    cout << "Ñîðòèðîâêà ìåòîäîì åñòåñòâåííîãî ñëèÿíèÿ" << endl;
    int p = 0;
    int i = 0;
    int j;
    int** a = new int* [r1];
...
{
    cout << "Ñîðòèðîâêà ìåòîäîì ñáàëàíñèðîâàííîãî ñëèÿíèÿ" << endl;
    int mid = r2 / 2; // íàõîäèì ñåðåäèíó ñîðòèðóåìîé ïîñëåäîâàòåëüíîñòè
    if (r2 % 2 == 1)
        mid++;
    int h = 1;
...
vector<int> PolyphaseMergeSort(vector<int> vec)
{
    ...
    cout << "Ðàçäåëåíèå íà ðàâíûå ñåðèè" << endl;
    PrintSeries(series);
    ...
    cout << "Ðàçäåëåíèå ñåðèé íà ôàéëû F1 è F2 (" << size1 << " " << size2 << ")" << endl;
    PrintFiles(F1, F2, F3);

    // sort elements inside series
    cout << "Ñîðòèðîâêà ýëåìåíòîâ âíóòðè ôàéëîâ" << endl;
...
{
    int arr[r1];
    setlocale(LC_ALL, "rus");// êèðèëëèöà
    for (int i = 0; i < r1; i++)
    {
        arr[i] = -100 + rand() % 200;
        cout << arr[i] << " ";
